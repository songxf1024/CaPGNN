...
class CommBuffer(object):
    ...
    '''
    def __init__(self, buffer_shape: List[int],
        ...
        self._f_avg: Test_Buffer_Type = []
        self._b_avg: Test_Buffer_Type = []
    ...
    *************************************************
    ...
    def _generate_test_buffer(self, send_idx: Dict[int, Tuple[int, int]], recv_idx: Basic_Buffer_Type):
        ...
        for dim_size in self.buffer_shape:
            ...
            send_temp_params: Dict[int, Tensor] = {}
            b_tmp: Dict[int, Tensor] = {}
            for pid, idx in send_idx.items():
                num_nodes = idx[1] - idx[0]
                send_temp_params[pid] = torch.zeros(size=(self.cache_size+1,), dtype=torch.int).pin_memory()
                send_temp_buffer[pid] = torch.zeros((num_nodes, dim_size), dtype=torch.float32).pin_memory()
                b_tmp[pid] = torch.zeros((num_nodes, dim_size), dtype=torch.float32, device=self.device)
                # b_tmp[pid] = torch.zeros((self.boundary[pid].shape[0], dim_size), dtype=torch.float32, device=self.device)
            self.test_send_buffers_cpu.append((send_temp_buffer, send_temp_params))
            self._b_avg.append(b_tmp)
        # generate the receiving buffer
        for dim_size in self.buffer_shape:
            recv_temp_buffer_cpu: Dict[int, Tensor] = {}
            recv_temp_buffer_gpu: Dict[int, Tensor] = {}
            recv_temp_params_cpu: Dict[int, Tensor] = {}
            recv_temp_params_gpu: Dict[int, Tensor] = {}
            f_tmp: Dict[int, Tensor] = {}
            for pid, idx in recv_idx.items():
                num_nodes = len(idx)
                recv_temp_params_cpu[pid] = torch.zeros(size=(self.cache_size+1,), dtype=torch.int).pin_memory()
                recv_temp_params_gpu[pid] = torch.zeros(size=(self.cache_size+1,), dtype=torch.int, device=self.device)
                recv_temp_buffer_cpu[pid] = torch.zeros((num_nodes, dim_size), dtype=torch.float32).pin_memory()
                recv_temp_buffer_gpu[pid] = torch.zeros((num_nodes, dim_size), dtype=torch.float32, device=self.device)
                f_tmp[pid] = torch.zeros((num_nodes, dim_size), dtype=torch.float32, device=self.device)
                # f_tmp[pid] = torch.zeros((self.recv_shape[pid], dim_size), dtype=torch.float32, device=self.device)
            self.test_recv_buffers_cpu.append((recv_temp_buffer_cpu, recv_temp_params_cpu))
            self.test_recv_buffers_gpu.append((recv_temp_buffer_gpu, recv_temp_params_gpu))
            self._f_avg.append(f_tmp)
        ...
        for layer in layer_keys:
            ...
            for pid, size_buffer in self.recv_original_size_buffers[layer].items():
                ...
                for qt_fp_size in size_buffer.values():
                    layer_qt_size += qt_fp_size[0]
                    layer_fp_size += qt_fp_size[1]
                    recv_qdata_cpu = torch.zeros(size=(layer_qt_size,), dtype=torch.int8).pin_memory()
                    recv_qparams_cpu = torch.zeros(size=(2, layer_fp_size), dtype=torch.bfloat16).pin_memory()
