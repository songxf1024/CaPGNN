    ...
    while True:
        ...
        for gpu_id in gpu_ids_list:
            result = subprocess.run(
                ['nvidia-smi', '-i', gpu_id, '--query-gpu=temperature.gpu', '--format=csv,noheader,nounits'],
        ...
        if all_below_threshold:
            ...
            break
        print(
            f'>> To prevent performance limitations from occurring in high temperatures of the GPU, wait for cooling({temp_threshold}°): ' + ' | '.join(
                temperatures), end='\r')
