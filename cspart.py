# Graph partitioning of computing power perception.
# Digit the molecular graph based on the complexity of the graph and the computing power of the GPU.
import csv
import os
import random
...
class CSPart(object):
    ...
    def coarse_load_patition(self):
        partition_dir = f'{self.partition_dir}/{self.dataset_name}/{self.server_num}server/server{self.server_id}/{self.partition_size}part'
    ...
    def overlapping_our(self, part_size, dataset, gpus_list, part_dir='data/part_data', k=-1, our_partition=False):
        ...
        partition_file = f'{part_dir}/{dataset}/{self.server_num}server/server{self.server_id}/{part_size}part/{dataset}_processed_partitions_{our_partition}_{sorted(gpus_list)}.pkl'
...
def partition1():
    ...
    # python cspart.py --dataset_index=4 --server_num=1 --partition_num=5 --our_partition=1
    # python cspart.py -p=1 -d=13 -s=1 -n=4 -g=0
    # If server_num>1, it indicates that there are multiple servers, and partition_num is a list representing the number of partitions on each server.
    # For example, [2,4] means server 1 has 2 GPUs and server 2 has 4 GPUs.
